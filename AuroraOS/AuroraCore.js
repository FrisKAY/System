...
var programData=[
  ...
  {name: "Home", url: "http://aurora.os/index", icon: "home", keywords: "home,homepage,index,information"},
  {name: "Sudoku Solver", url: "http://aurora.os/sudokuSolver", icon: "sudokusolver", keywords: "sudoku,solver,games,interactive"},
  {name: "Mandelbrot", url: "http://aurora.os/mandelbrot", icon: "mandelbrot", keywords: "mandelbrot,julia,set,generator,fractal,interactive,math,canvas"},
  ...
  {name: "HTML Editor", url: "http://aurora.os/editor", icon: "htmleditor", keywords: "html,editor,css,interactive,gadget"},
  {name: "Bézier", url: "http://aurora.os/bezier", icon: "bezier", keywords: "bezier,bézier,interactive,gadget"},
  {name: "Is it Prime?", url: "http://aurora.os/isitprime", icon: "isitprime", keywords: "prime,generator,math,information"},
  {name: "N:th Prime", url: "http://aurora.os/nthPrime", icon: "nthprime", keywords: "nth,prime,generator,math,information"},
  {name: "reddit Live 2.0", url: "http://aurora.os/redditLive", icon: "redditlive", keywords: "reddit,live,api,information"},
  {name: "Egg Hunt", url: "http://aurora.os/egghunt", icon: "egghunt", keywords: "egg,hunt,confused,travolta,game,reddit,easter"},
  ...
  {name: "Hit Lawyer", url: "http://aurora.os/hitLawyer", icon: "hitlawyer", keywords: "hit,lawyer,gadget"},
  {name: "Fractal", url: "https://aquaplexus.github.io/fractal", icon: "fractal", keywords: "fractal,generator,interactive,math"},
  {name: "Multiples", url: "http://aurora.os/multiples", icon: "multiples", keywords: "multiples,math,interactive"},
  {name: "Fireworks", url: "https://aquaplexus.github.io/firework", icon: "fireworks", keywords: "fireworks,interactive,gadget,canvas"},
  {name: "Phone Snake", url: "http://aurora.os/phonesnake", icon: "phonesnake", keywords: "phone,snake,game,interactive"},
  {name: "Back Dropper", url: "http://aurora.os/backdropper", icon: "backdropper", keywords: "back,dropper,library,background,canvas"},
  {name: "Parrots", url: "http://aurora.os/parrots", icon: "parrots", keywords: "parrots,dank,reddit,party,epilepsy"},
  {name: "Smoke", url: "http://aurora.os/smoke", icon: "smoke", keywords: "smoke,3d,canvas,math"},
  {name: "404.html", url: "http://aurora.os/404", icon: "404", keywords: "404,terminal,console,greentext"},
  {name: "Matrix", url: "http://aurora.os/matrix", icon: "matrix", keywords: "matrix,math,multiplication"},
  {name: "Dodge", url: "http://aurora.os/games/dodge", icon: "dodge", keywords: "game,reddit,cursor,slide"}
];
...
function setup(){
  ...
  for (var i=0;i<programData.length;i++){
    ...
    el.innerHTML="<div class=\"dimg\" style=\"background-image:url(http://aurora.os/images/win_icons/"+programData[i].icon+".png)\"></div> <p class=\"ddesc\">"+programData[i].name+"</p>";
...
function addWindow(id,title,contStr,w,h){
  ...
  var inner="<div class=\"infobar infoCol\" id=\""+winCount+"\"><div class=\"close\" title=\"Close\" onclick=closeWin("+winCount+")>✕</div><div class=\"max\" title=\"Toggle\" onclick=toggle("+winCount+")>◻</div><div class=\"min\" title=\"Minimize\" onclick=minWin("+winCount+")>_</div><div class=\"reload\" title=\"Reload\" onclick=reloadWin("+winCount+")>↻</div><div class=\"winicon\" style=\"background-image:url(http://aurora.os/images/win_icons/"+data.icon+".png)\"></div><div class=\"wintitle\">"+data.name+"</div></div><iframe class=\"content\" src=\""+(id==0?tmpUrl:"")+"\"></iframe><div class=\"loadingoverlay\"><div id=\"loader\"></div><div id=\"loader2\"></div><div id=\"loader3\"></div></div>"+err+"<div class=\"resize\" scale=\"lw\"></div><div class=\"resize\" scale=\"lwh\"></div><div class=\"resize\" scale=\"h\"></div><div class=\"resize\" scale=\"wh\"></div><div class=\"resize\" scale=\"w\"></div>";
...
function addTaskbarIcon(id,count,actLvl,name,icon,stickied){
  ...
  iconEl.innerHTML="<div class=\"smallicon\" style=\"background-image:url(http://aurora.os/images/win_icons/"+icon+".png);\"></div>";
...
function fillHome(){
  ...
  if (locData[0].oftenUsed>0){
    ...
    for (var i=0;i<6;i++){
      ...
      output+="<div class=\"pbitem\" onclick=selectIcon(event,"+locData[i].index+",true)><div id=\"pbicon\" style=\"background-image:url(http://aurora.os/images/win_icons/"+locData[i].icon+".png)\"></div><div id=\"pbtitle\">"+locData[i].name+" <span style=\"color:#666\">("+locData[i].oftenUsed+")</span></div></div>";
    }
  ...
  for (var i=0;i<locData.length;i++){
    ...
    output+="<div class=\"pbitem\" onclick=selectIcon(event,"+locData[i].index+",true)><div id=\"pbicon\" style=\"background-image:url(http://aurora.os/images/win_icons/"+locData[i].icon+".png)\"></div><div id=\"pbtitle\">"+locData[i].name+"</div></div>";
  }
  ...
  for (var i=1;i<programData.length;i++){
    ...
    if (newArr.length>0){
      ...
      results.push({
        ...
        icon: "http://aurora.os/images/win_icons/"+programData[i].icon+".png"
      });
    }
  }
...
function colorSelect(){
  ...
  for (var i=1;i<backgrounds.length+1;i++){
    inner+="<div class='backoption' "+(i==backId?"selected":"")+" onclick=selIco("+i+") style='background-image:url(http://aurora.os/images/wallpapers/icons/"+(backgrounds[i-1].replace(".","Ico."))+")'></div>";
  }
